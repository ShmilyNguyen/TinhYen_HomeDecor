    ...
    public partial class region : System.Web.UI.Page
    {
        ...
        protected void RadGrid1_ItemCommand(object sender, GridCommandEventArgs e)
        {
            try
            {
                ...
                if (e.CommandName == "Update")
                {
                    ...
                    try
                    {
                        ...
                        if (id == 0)
                        {
                            ...
                            clsCodeMaster.GenCode("region", UserID, out ID, out Code);
                            id = int.Parse(ID);
                        }



                        string storeProc = "[usp_InsertUpdateregion]";
                        int result = 0;
                        try
                        {
                            using (SqlConnection conn = new SqlConnection(clsCommon.strCon))
                            {
                                ...
                                if (id > 0)
                                {
                                    ...
                                    cmd.Parameters.AddWithValue("@region_code", code);
                                    cmd.Parameters.AddWithValue("@region_name", name);
                ...
                {
                    string id = (e.Item as GridDataItem).OwnerTableView.DataKeyValues[e.Item.ItemIndex]["region_id"].ToString();
                    string sQuery = "delete from [region] where [region_id]=" + id;
                    int result = 0;
                    try
                    {
                        using (SqlConnection conn = new SqlConnection(clsCommon.strCon))
