...
namespace WKS.DMS.WEB.Forms
{
    public partial class area : System.Web.UI.Page
    {
        ...
        public DataTable GetData()
        {
            ...
            string sQuery = @"select * from area";
        ...
        protected void RadGrid1_ItemCommand(object sender, GridCommandEventArgs e)
        {
            try
            {
                ...
                if (e.CommandName == "Update")
                {
                    ...
                    try
                    {
                        ...
                        if (id == 0)
                        {
                            ...
                            clsCodeMaster.GenCode("area", UserID, out ID, out Code);
                            id = int.Parse(ID);
                        }



                        string storeProc = "[usp_InsertUpdatearea]";
                        int result = 0;
                        try
                        {
                            using (SqlConnection conn = new SqlConnection(clsCommon.strCon))
                            {
                                ...
                                if (id > 0)
                                {
                                    ...
                                    cmd.Parameters.AddWithValue("@area_code", code);
                                    cmd.Parameters.AddWithValue("@area_name", name);
                ...
                {
                    string id = (e.Item as GridDataItem).OwnerTableView.DataKeyValues[e.Item.ItemIndex]["area_id"].ToString();
                    string sQuery = "delete from [area] where [area_id]=" + id;
                    int result = 0;
                    try
                    {
                        using (SqlConnection conn = new SqlConnection(clsCommon.strCon))
